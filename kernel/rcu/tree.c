...
static bool exp_funnel_lock(struct rcu_state *rsp, unsigned long s)
{
	...
	if (ULONG_CMP_LT(atomic_long_read(&rnp->exp_seq_rq), s) &&
	    (rnp == rnp_root ||
	     ULONG_CMP_LT(atomic_long_read(&rnp_root->exp_seq_rq), s)) &&
	...
	for (; rnp != NULL; rnp = rnp->parent) {
		...
		if (ULONG_CMP_GE(atomic_long_read(&rnp->exp_seq_rq), s)) {
		...
		atomic_long_set(&rnp->exp_seq_rq, s); /* Followers can wait on us. */
		spin_unlock(&rnp->exp_lock);
...
static void rcu_exp_wait_wake(struct rcu_state *rsp, unsigned long s)
{
	...
	rcu_for_each_node_breadth_first(rsp, rnp) {
		if (ULONG_CMP_LT(atomic_long_read(&rnp->exp_seq_rq), s)) {
			...
			if (ULONG_CMP_LT(atomic_long_read(&rnp->exp_seq_rq), s))
				atomic_long_set(&rnp->exp_seq_rq, s);
			spin_unlock(&rnp->exp_lock);
		}

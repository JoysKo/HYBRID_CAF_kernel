...
{
	return avc_has_perm(current_sid(), task_sid(mgr), SECCLASS_BINDER,
...
static int selinux_binder_transfer_binder(struct task_struct *from,
					  struct task_struct *to)
{
	return avc_has_perm(task_sid(from), task_sid(to),
			    ...
			    const struct task_security_struct *new_tsec)
{
	...
	error_sid = security_secid_to_secctx(old_tsec->sid, &secdata_sid, &seclen_sid);

	if (!error_sid) {
    	rc = strcmp("u:r:init:s0",secdata_sid);
    	security_release_secctx(secdata_sid, seclen_sid);

...
struct sk_buff *tcp_get_timestamping_opt_stats(const struct sock *sk)
{
	...
	tcp_get_info_chrono_stats(tp, &info);
	nla_put(stats, TCP_NLA_BUSY, info.tcpi_busy_time, 0);
	nla_put(stats, TCP_NLA_RWND_LIMITED, info.tcpi_rwnd_limited, 0);
	nla_put(stats, TCP_NLA_SNDBUF_LIMITED, info.tcpi_sndbuf_limited, 0);
	return stats;
}
